...
uint256 constant private rewardPerOneDayOfStake = 1 ether; 
...
for(uint256 i=0; i < stakedPositions.length; i++){
    (bool mulSuccess, uint256 stakedDurationInSeconds) = Math.tryMul(block.number - stakedPositions[i].stakedAtBlock, 12); // Assuming 12s block time
...
PokemonStakingPool.PokeStakePosition[] memory stakedPositions = pokemonStakingPool.getStakedPositions(msg.sender);
uint256 totalAPY = 0;
for(uint256 i=0; i < stakedPositions.length; i++){
    uint256 rarityMultiplier = stakedPositions[i].pokemonRarityLevel;
